...
I2C i2c(I2C_SDA, I2C_SCL); // Configure I2C

// Set initial sensor scales
enum Ascale {
  AFS_2G = 0,
  AFS_4G,
  AFS_8G,
...
float ax,ay,az;
float gx,gy,gz;
int16_t accelCount[3],gyroCount[3],tempCount;
...
{  
 ...
 // wake up device
  ...
  thread_sleep_for(100);  // Delay 100 ms for PLL to get established on x-axis gyro; should check for PLL ready interrupt  
...
void MPU6050::MadgwickFilter(float ax, float ay, float az, float gx, float gy, float gz)
{
